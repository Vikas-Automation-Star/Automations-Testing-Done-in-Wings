...
public class ProductWiseDebitNote extends TransactionsBaseClass {
    WindowsDriver driver;
    Common common;
    String dataFile;
    boolean discountIsClicked=false;

    public ProductWiseDebitNote(WindowsDriver driver, String file) {
        super(driver);
        this.driver = driver;
    ...
    public void productWiseDebitNote(String invoiceNum,String payableVoucher,String tempAPIBodyUpdate,String apiResponse,String outputFile) throws Exception {
        long productWiseDebitNote = System.nanoTime();

        navigateToMastersWhen3Steps("Finance", "Party Adjustments", "Product Wise Debit Note");
        Thread.sleep(1000);
...
//        APIClient.validateAPIWithExcel(newVoucherID, tempAPIBodyUpdate, apiResponse, outputFile, "CreditNoteOnCustomer");
        ...
        long creditNoteOnCustomersEnd = System.nanoTime() - productWiseDebitNote;
        FileUtil.writeTimeLogInMinutes("Credit note on customers End at:- ", creditNoteOnCustomersEnd);
    }

    public void items() throws InterruptedException, IOException, ParseException, AWTException {
        ...
        for (int i = 0; i < priceType.size()-1; i++) {
            enterListData(productBatchRow.get(i), dataFile, "Items", "ProductBatchId", i);
...
//        common.clickElement("xpath", "//Header[@Name='GST Amount']");
        ...
        List<WebElement>  tdsAct= common.findWebElements("xpath", "//Table[@Name='Accounts']/*[contains(@Name,'Row ')]/Edit[starts-with(@Name,'TDS Account Row ')]");
        List<WebElement> tdcAmt= common.findWebElements("xpath", "//Table[@Name='Accounts']/*[contains(@Name,'Row ')]/Edit[starts-with(@Name,'TDS Amount Row ')]");
        ...
        for (int i = 0; i < cashTab.size(); i++) {
            ...
            enterListData(tdsAct.get(i), dataFile, "Accounts", "TDSAccount", i);
            enterListData(tdcAmt.get(i), dataFile, "Accounts", "TDSAmount", i);
    ...
    public void billsPayable(String receivableNumber) throws IOException, InterruptedException {
        common.clickElement("xpath", "//TabItem[contains(@Name,'  BillsPayable  ')]");
        adjustAmountInBillsPayable(dataFile,receivableNumber);
    }
