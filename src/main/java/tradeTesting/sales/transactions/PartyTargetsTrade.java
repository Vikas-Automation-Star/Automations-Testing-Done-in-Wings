...
public class PartyTargetsTrade  extends TransactionsBaseClass {
    WindowsDriver driver,rootDriver;
    Common common;
    String dataFile;

    public PartyTargetsTrade(WindowsDriver driver, String file) {
        super(driver);
        common = new Common(this.driver = driver);
        dataFile = file;
    }

    public String partyTargetsTrade(String tempAPIBodyUpdate,String apiResponse,String outputFile) throws Exception {
        long start = System.nanoTime();
        System.out.println("Party Targets:"+start);
        Thread.sleep(100);
        long generalInfoStart=System.nanoTime();
        navigateToMastersWhen3Steps("Sales","Targets","Party Targets" );
        Thread.sleep(3000);
        ...
        FileUtil.writeTimeLogInMinutes("Party Targets General Information End:- ", generalInfoEndTime);

        long addProductStart = System.nanoTime();
        addProduct();
        long addProductEnd = System.nanoTime() - addProductStart;
        FileUtil.writeTimeLogInMinutes("Party Targets Add Products:- ", addProductEnd);
...
//            APIClient.validateAPIWithExcel(newVoucherID,tempAPIBodyUpdate,apiResponse,outputFile,"defineSalesTarget");
        ...
        FileUtil.writeTimeLogInMinutes("Party Targets Transaction Ended at:- ", defineSalesTargetEnd);
    ...
    public void addProduct() throws Exception {
        List<String> productCode=readExcelData(dataFile,"Targets","PartyAccount");
        for (int i = 0; i < productCode.size() ; i++)   {
            addData("xpath","//Edit[@Name='Party Account * Row "+i+", Not sorted.']",dataFile,"Targets","PartyAccount",i);
        }
        ...
        for (int i = 0; i < productCode.size() ; i++) {
            enterListData(division.get(i), dataFile, "Targets", "Division1", i);
