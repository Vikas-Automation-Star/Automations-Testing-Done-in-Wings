...
public class PVAMR_UnRegInterInclusive extends Transaction {
    WindowsDriver driver;
    Common common;
    String dataFile;
    boolean gstAmountClicked=false;
    public PVAMR_UnRegInterInclusive(WindowsDriver driver, String file) {
        super(driver);
        this.driver = driver;
    ...
    public String[] pvamr_UnRegInterInclusive(String voucherNum) throws InterruptedException, IOException, ParseException, AWTException {
        navigateToMastersWhen3Steps(common.getData(dataFile,"PVAMR UNRegInter","menu"),common.getData(dataFile,"PVAMR UNRegInter","menuItem"), common.getData(dataFile,"PVAMR UNRegInter","subMenuItem"));
        Thread.sleep(1000);
...
//        gstTransactionType("Intra State Purchase from Registered Dealers");
        Thread.sleep(1500);
        selectPendingsSalesOrder(voucherNum,common.getData(dataFile,"PVAMR UNRegInter","FYear"));
        enterInput("xpath","//Edit[@Name='Purchase A/C Code']",dataFile,"PVAMR UNRegInter","purchaseA/cCode");
        enableCheckboxSelection("//CheckBox[@Name='Apply TCS']");
        enterInput("xpath","//Edit[@Name='TCS Trans Nature']", dataFile,"PVAMR UNRegInter","tcsNature");
        inputTextWithValidation("xpath", "//Edit[@Name='Supplier Bill No *']", common.getData(dataFile,"PVAMR UNRegInter", "supplierCode") +common.getRandom());
        inputTextWithValidation("xpath", "//Edit[@Name='Supplier Bill Date *']", common.getData(dataFile,"PVAMR UNRegInter", "date") + Time.timeStamp());
        enterInput("xpath","//Edit[@Name='Batch Policy']",dataFile,"PVAMR UNRegInter","batchPolicy");
            ...
            for (String productCode : productCodes) {
                if (value != null && value.contains(productCode) && !processedCodes.contains(productCode)) {
                    if (processedCodes.isEmpty()) {
                        ...
                        Thread.sleep(1000);
                        enterData("xpath","//Edit[@Name='Quantity Row 2, Not sorted.']",dataFile,"PVAMR UNRegInter","serialQuantity");
                    }
        ...
        UnRegOtherChargesCalculations(dataFile,"PVAMR UNRegInter", "otherChargesCode", "amount", "rowCount","HSNCode","TaxType","expectedTaxableValue","expectedGstValue","expectedNetValue");
